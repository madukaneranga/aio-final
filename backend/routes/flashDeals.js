...
router.get("/current", async (req, res) => {
  try {
    // Try to get current active deal
    ...
    // Increment view count only if a deal exists
    if (flashDeal) {
...
router.get("/", async (req, res) => {
  try {
    const { page = 1, limit = 10, status = "all", store = null } = req.query;
...
router.get("/:id/analytics", async (req, res) => {
  try {
    ...
    const analytics = {
      ...
      clickThroughRate:
        flashDeal.viewCount > 0
          ? ((flashDeal.clickCount / flashDeal.viewCount) * 100).toFixed(2)
          : 0,
